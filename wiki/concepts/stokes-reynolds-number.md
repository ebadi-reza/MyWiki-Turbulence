...
(1991a), Ozdemir et al. (2014), Jensen et al. (1989).
...
Transition is governed by `Re_s ∝ Re/W`, i.e. a critical Reynolds number `Re_c = f(W)`. For **reciprocating** flow the relationship is **linear and well established: `Re_c = 700 W`** (equivalently `Re_s ≃ 500`). For **pulsatile** flow the `Re_c`–`W` relationship is complicated and transition can be advanced or delayed by `W` ([[2016-ebadi-thesis]] §7.3; see [[pulsatile-flow]]).
...
- In [[2016-ebadi-ictam]]: `Re_s = 648` is type III, `Re_s = 1019` is type IV, and the III→IV transition (emergence of the [[internal-shear-layer]]) requires **Re_s > 750**.
